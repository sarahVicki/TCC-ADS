...
class SaloesSupabaseUser extends BaseAuthUser {
  SaloesSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> saloesSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SaloesSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
